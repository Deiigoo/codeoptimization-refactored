...
public class FantasyCalculatorTest {
    ...
    @Test
    public void testGoalkeeperNormalMatch() {
        PlayerStats s = new PlayerStats();
        s.position = "PORTERO";
        s.minutes = 90;        // >=60 mins = 5 pts
        s.goals = 0;           // 0 pts
        ...
        s.saves = 3;           // 3 * 1 = 3 pts
        s.goalsAgainst = 0;    // 0 goals = 5 pts
        s.yellowCard = true;  // 0 pts
        s.redCard = false;     // -5 pts
        s.matchResult = 'G';   // Win = 5 pts
        
        // Expected: 5 + 6 + 3 + 5 + 5 - 5 + 5 = 21
        assertEquals(21, FantasyCalculator.calculatePoints(s), "GK Normal Match calculation failed");
    }
}
